...
class DirectIngredientProcessor:
    ...
    def _load_spoonacular_config(self) -> Dict[str, str]:
        """Load Spoonacular API configuration"""
        config_path = os.path.join(os.path.dirname(__file__), '..', '..', 'config', 'spoonacular.json')
        try:
            with open(config_path, 'r') as f:
                return json.load(f)
        except FileNotFoundError:
            print(f"❌ Spoonacular config not found at {config_path}")
            return {"api_key": ""}
...
Return ONLY the JSON, no other text."""
            ...
            try:
                ...
                if all(field in cost_info for field in required_fields):
                    self._log_and_print(f"   💰 AI estimated cost: ${cost_info['cost_per_unit']:.2f}/{cost_info['cost_unit']}")
                    return cost_info
                else:
                    self._log_and_print(f"   ⚠️ AI cost estimation missing required fields for '{ingredient_name}'")
                    return None
