#!/usr/bin/env python3
"""
Direct Recipe Processor - Autonomous Recipe Ingestion Pipeline
...
class DirectRecipeProcessor:
    ...
    def _load_configuration(self) -> Dict[str, str]:
        ...
        # Load spoonacular config
        spoon_config_path = os.path.join(os.path.dirname(__file__), '..', '..', 'config', 'spoonacular.json')
        try:
            with open(spoon_config_path, 'r') as f:
                spoon_config = json.load(f)
                env_vars.update(spoon_config)
        except FileNotFoundError:
            print(f"⚠️  Spoonacular config not found at {spoon_config_path}")
